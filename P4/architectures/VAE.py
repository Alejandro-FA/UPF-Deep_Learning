...
class VAEEncoder(nn.Module):
    def __init__(self, out_features, base_channels=16, drop_ratio=0):
        super(VAEEncoder, self).__init__()
        
        self.layer1 = ConvBNReLU(1, base_channels, pooling=False, drop_ratio=drop_ratio)
        self.layer2 = ConvBNReLU(base_channels, base_channels * 2, pooling=True, drop_ratio=drop_ratio)
        self.layer3 = ConvBNReLU(base_channels * 2, base_channels * 4, pooling=True, drop_ratio=drop_ratio)
...
class VAEDecoder(nn.ModuleDict):
    def __init__(self, out_features, base_channels=16, drop_ratio=0):
        ...
        self.dropout = nn.Dropout(p=drop_ratio)
        self.layer3 = BNReLUConv(base_channels * 4, base_channels * 2, pooling=True, drop_ratio=drop_ratio)
        self.layer2 = BNReLUConv(base_channels * 2, base_channels, pooling=True, drop_ratio=drop_ratio)
        self.layer1 = BNReLUConv(base_channels, 1, pooling=False, drop_ratio=drop_ratio)
...
class VAE(nn.Module, GenerativeModel):
    def __init__(self, out_features=32, base_channels=16):
        ...
        self.encoder = VAEEncoder(out_features * 2, base_channels, drop_ratio=0.2)
        self.decoder = VAEDecoder(out_features, base_channels, drop_ratio=0.2)
