...
class Encoder(nn.Module):
    def __init__(self, out_features, base_channels=16, is_gan = False):
        super(Encoder, self).__init__()
        if is_gan:
            self.layer1 = ConvBNReLU(1, base_channels, pooling=False, use_bn=False)
        else:
...
class Decoder(nn.Module):
    def __init__(self, out_features, base_channels=16, is_gan = False):
        ...
        if is_gan:
            self.layer1 = BNReLUConv(base_channels, 1, pooling=False, use_bn=False)
        else:
